...
function acctInfo(httpkey, httpsecret, passphrase) {
    ...
    _instance.authClient = new AuthenticatedClient(httpkey, httpsecret, passphrase);
...
function initOrderData() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (start) {
            ...
            try {
                if (from) {
                    res = yield self.authClient.spot().getOrdersPending({ 'instrument_id': config.instrument_id, 'limit': limit, 'from': from });
                }
                else {
                    res = yield self.authClient.spot().getOrdersPending({ 'instrument_id': config.instrument_id, 'limit': limit });
                }
                ...
                yield sleep(100); //每秒10次 限制是每2秒20次
                if (res.length < limit) {
                    start = false;
                }
            }
