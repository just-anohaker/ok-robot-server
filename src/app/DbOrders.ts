...
export class DbOrders {
    constructor(private handler: Sqlite3.Database, private acct: any) { }
    ...
    add(newOrder: Order): boolean {
        ...
        try {
            const stmt = this.handler.prepare(
                `INSERT INTO orders (order_id,client_oid, created_at, filled_notional, filled_size
    ...
    addBatchOrder(newOrder: Order): boolean {
        ...
        try {
            const stmt = this.handler.prepare(
                `INSERT INTO orders (order_id,client_oid, acct_key,instrument_id)
